...
public class PostService {
    ...
    public PostEntity view(PostViewRequest postViewRequest) {
        ...
        return postRepository.findFirstByIdAndStatusOrderByIdDesc(postViewRequest.getPostId(), "REGISTERED")
                .map( it -> {
                    // entity 존재
                    if (!it.getPassword().equals(postViewRequest.getPassword())){ // 해당 데이터가 있을 때만 실행
                        var format = "패스워드가 맞지 않습니다 %s vs %s"; // 패스워드 비교
                        throw new RuntimeException(String.format(format, it.getPassword(), postViewRequest.getPassword()));
                    }

                    // 답변 글도 같이 적용
                    /*var replyList = replyService.findByPostId(it.getId());
                    it.setReplyList(replyList);*/


                    return it; // 맞으면 해당 엔티티 리턴

                }).orElseThrow( // 데이터가 없을 시 실행
                        ()-> {
